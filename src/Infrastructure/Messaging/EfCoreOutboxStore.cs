...
public sealed class EfCoreOutboxStore(BlogDbContext dbContext) : IOutboxStore
{
    ...
    {
        var message = await dbContext.OutboxMessages.FirstAsync(x => x.Id == id, cancellationToken);
        message.RetryCount++;
